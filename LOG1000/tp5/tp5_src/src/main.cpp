...
int main(int argc, char **argv)
{
    ...
    while(choice!=0)
    {
        ...
        case 2:
        {
            ...
            std::vector<std::string> nurse = nurses.select();
            if (appointments.schedule(loggedInUser->profileID, nurse[NURSES_USER_ID])) 
            {
                std::cout << "\nVotre rendez-vous a été correctement enregistré";
                std::cout << "\nVous serez facturé "<< appointments.getHoursRate(loggedInUser->disease, std::stoi(nurse[NURSES_EXPERIENCE_YEARS]) ) << " dollars";
            }
            else
            {
