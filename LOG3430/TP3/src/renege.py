...
class RENEGE:
    ...
    def process_email(self, new_emails, log, prob, param):
        """
        ...
        for email in emails["dataset"]:
            ...
            date = data["Date"]
            body = data["Body"]
            is_spam = self.e_mail.is_spam(subject, body, log, prob, param)
    ...
    def update_user_info(self, user_id, new_user_date, new_email_spam, new_email_ham):
        ...
        if new_date > self.crud.get_user_data(user_id, "Date_of_last_seen_message"):
            if not self.crud.update_users(
                user_id, "Date_of_last_seen_message", new_user_date
            ):
                return False
    ...
    # index est l'index de l'utilisateur
    def calculate_trust(self, index):
        date_of_first_message = self.crud.get_user_data(
            index, "Date_of_first_seen_message"
        )
        date_of_last_message = self.crud.get_user_data(
            index, "Date_of_first_seen_message"
        )
        nham = self.crud.get_user_data(index, "HamN")
        nspam = self.crud.get_user_data(index, "SpamN")
        trust1 = date_of_last_message * nham / date_of_first_message * (nham + nspam)
        user_groups = self.crud.get_user_data(index, "Groups")
        group_counter = 1
        total = 0

        for group in user_groups:
            group_id = self.crud.get_group_id(group)
            total = total + self.crud.get_groups_data(group_id, "Trust")
            group_counter += 1

        trust2 = total / group_counter
        ...
        if trust1 > 100:
            trust = 100
        if trust > 100 or trust < 0:
            raise Exception("Incoherent trust values")
