...
void Test::setUp()
{
    ...
    this->_appointments = new Appointments(CSV_TEST_FILE_PATH);
}
...
void Test::testGetHoursRate()
{
    CPPUNIT_ASSERT(this->_appointments->getHoursRate("covid-19", 10) == 2);
    CPPUNIT_ASSERT(this->_appointments->getHoursRate("grippe", 1) == 20);
    CPPUNIT_ASSERT(this->_appointments->getHoursRate("fievre", 4) == 30);
    CPPUNIT_ASSERT(this->_appointments->getHoursRate("mal de tete", 10) == 36);
}
