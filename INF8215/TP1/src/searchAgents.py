...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not hitsWall:
                for item in state[1]:
                    if not item == (nextx, nexty):
                        unvisited.append(item)
                successors.append((((nextx, nexty), unvisited), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for item in state[1]:
        if xy1 == item:
            return 0
        distArray.append(((abs(xy1[0] - item[0]) + abs(xy1[1] - item[1])), item))
...
def foodHeuristic(state, problem: FoodSearchProblem):
    ...
    for item in foodList:
        if position == item:
            return 0
        distArray.append((abs(position[0] - item[0]) + abs(position[1] - item[1]), item))
    sor = sorted(distArray, reverse=True)
    food = sor[0][1]

    problem1 = PositionSearchProblem(problem.startingGameState, start=position, goal=food, warn=False, visualize=False)
    distance = problem1.getCostOfActions(search.astar(problem1, heuristic=manhattanHeuristic))
